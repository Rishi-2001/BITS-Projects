...
def generate_training_data(display, clock):
    ...
    for _ in tqdm(range(training_games)):
        ...
        for _ in range(steps_per_game):
            ...
            direction, button_direction = generate_random_direction(
                snake_position, angle
            )
            (
            ...
            direction, button_direction, training_data_y = generate_training_data_y(
                snake_position,
                angle_with_apple,
                button_direction,
                direction,
    ...
    if direction == -1:
        if is_left_blocked == 1:
            if is_front_blocked == 1 and is_right_blocked == 0:
                direction, button_direction = direction_vector(
                    snake_position, angle_with_apple, 1
                )
                training_data_y.append([0, 0, 1])
            elif is_front_blocked == 0 and is_right_blocked == 1:
                direction, button_direction = direction_vector(
                    snake_position, angle_with_apple, 0
                )
