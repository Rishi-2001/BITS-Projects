...
class Data:
    ...
    def get_batches(self):
        num_batches = np.prod(self.in_text.shape) // (
            self.flags.seq_size * self.flags.batch_size
        )
        for i in range(0, num_batches * self.flags.seq_size, self.flags.seq_size):
            yield self.in_text[:, i : i + self.flags.seq_size], self.out_text[
                :, i : i + self.flags.seq_size
            ]
        ...
        for epoch in tnrange(1, epochs + 1, desc="Epoch Loop"):
            ...
            for x, y in batches:
                ...
                _ = torch.nn.utils.clip_grad_norm_(
                    self.parameters(), self.flags.gradients_norm
                )
                # self.Optimizer.decay(self.loss)

                self.optimizer.step()
            callback = "Loss: {} Time Taken : {}".format(
                self.loss_value, time.time() - start
            )
            self.scheduler.step()
            print(callback)
