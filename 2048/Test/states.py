...
class State:
    ...
    def game_over_look(self, matrix):
        ...
        if any(2048 in row for row in matrix):
            ...
            and not self.horizontal_move_exits_look(matrix)
            and not self.vertical_move_exits_look(matrix)
        ):
            return 1
        return 2
    ...
    def moveleft(self):
        ...
        self.left.gamegrid, self.left.score, tempflagcombine = self.combine(
            self.left.gamegrid, self.score
        )
        self.left.gamegrid, _ = self.stack(self.left.gamegrid)
        if tempflagstack or tempflagcombine:
            self.left.gamegrid, self.left.tilesum = self.add_new_tile(
                self.left.gamegrid, self.left.tilesum
            )
        else:
            self.left = None
            return
    ...
    def moveright(self):
        ...
        if tempflagstack or tempflagcombine:
            self.right.gamegrid, self.right.tilesum = self.add_new_tile(
                self.right.gamegrid, self.right.tilesum
            )
        else:
            self.right = None
            return
    ...
    def moveup(self):
        ...
        self.up.gamegrid, self.up.score, tempflagcombine = self.combine(
            self.up.gamegrid, self.score
        )
        self.up.gamegrid, _ = self.stack(self.up.gamegrid)
        self.up.gamegrid = self.transpose(self.up.gamegrid)
        if tempflagstack or tempflagcombine:
            self.up.gamegrid, self.up.tilesum = self.add_new_tile(
                self.up.gamegrid, self.up.tilesum
            )
        else:
            self.up = None
            return
    ...
    def movedown(self):
        ...
        self.down.gamegrid, self.down.score, tempflagcombine = self.combine(
            self.down.gamegrid, self.score
        )
        self.down.gamegrid, _ = self.stack(self.down.gamegrid)
        self.down.gamegrid = self.reverse(self.down.gamegrid)
        self.down.gamegrid = self.transpose(self.down.gamegrid)
        if tempflagstack or tempflagcombine:
            self.down.gamegrid, self.down.tilesum = self.add_new_tile(
                self.down.gamegrid, self.down.tilesum
            )
        else:
            self.down = None
            return
...
class History:
    ...
    def pop(self):
        ...
        self.index -= 1
        return (
            self.history[self.index - 1][0].gamegrid,
            self.history[self.index - 1][0].score,
            self.history[self.index - 1][0].tilesum,
        )
