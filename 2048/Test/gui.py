...
class GUI(tk.Frame):
    ...
    def make_GUI(self):
        ...
        for i in range(4):
            ...
            for j in range(4):
                cell_frame = tk.Frame(
                    self.main_grid,
                    bg=self.colors.EMPTY_CELL_COLOR,
                    width=120,
                    height=120,
                )
        ...
        self.game_over_frame = tk.Frame(
            self.main_grid, width=480, height=480, borderwidth=2
        )
        ...
        self.scorename_label = tk.Label(
            score_frame,
            bg=self.colors.UIBACK,
            fg=self.colors.SCORECOLOR,
            text="Score",
            font=self.colors.SCOREFONT,
        )
        ...
        self.title_frame_label = tk.Label(
            title_frame,
            bg=self.colors.UIBACK,
            fg=self.colors.SCORECOLOR,
            text="2048",
            font=self.colors.TITLEFONT,
        )
        self.title_frame_label.grid(row=0, sticky="w")
        self.title_sublabel = tk.Label(
            title_frame,
            bg=self.colors.UIBACK,
            fg=self.colors.SCORECOLOR,
            text="Use WASD or Arrow Keys to Play",
            font=self.colors.SUBTITLEFONT,
        )
        self.title_sublabel.grid(row=1, sticky="w")
        self.title_sublabel2 = tk.Label(
            title_frame,
            bg=self.colors.UIBACK,
            fg=self.colors.SCORECOLOR,
            text="Z to Undo, E to Lookahead Into Future States",
            font=self.colors.SUBTITLEFONT,
        )
        self.title_sublabel2.grid(row=3, sticky="w")
        self.title_sublabel3 = tk.Label(
            title_frame,
            bg=self.colors.UIBACK,
            fg=self.colors.SCORECOLOR,
            text="Use Q to Quit, Use R to Restart Game",
            font=self.colors.SUBTITLEFONT,
        )
        self.title_sublabel3.grid(row=2, sticky="w")
        self.title_sublabel4 = tk.Label(
            title_frame,
            bg=self.colors.UIBACK,
            fg=self.colors.SCORECOLOR,
            text="Use P to Print History, Use I to Print StateInfo",
            font=self.colors.SUBTITLEFONT,
        )
            ...
            for j in range(4):
                ...
                if cell_value == 0:
                    ...
                    self.cells[i][j]["number"].configure(
                        bg=self.colors.EMPTY_CELL_COLOR, text=""
                    )
                else:
                    if cell_value == -1:
    ...
    def popup(self):
        ...
        return tkinter.simpledialog.askstring(
            "Input Lookahead Steps",
            "How many steps ahead do you wish to look\n(Large Number may result in memory issues, keep < 5 for easy output)\n(Output states possible to console, default to 3 if cancelled or nothing is entered)",
            parent=self.main_grid,
        )
