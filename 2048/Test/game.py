...
class Game:
    ...
    def left(self, event):
        ...
        if not (self.history.history[self.history.index - 1][0].left is None):
            ...
            self.history.push(
                self.history.history[self.history.index - 1][0].left, "left"
            )
            self.gui.update_GUI(self.matrix, self.score)
            self.changeflag = False
            return
        ...
        if self.changeflag == True:
            self.stack()
            self.add_new_tile()
            self.history.history[self.history.index - 1][0].left = State(
    ...
    def up(self, event):
        ...
        if self.changeflag == True:
            self.add_new_tile()
            self.history.history[self.history.index - 1][0].up = State(
                self.matrix, self.score, self.tilesum
            )
            self.history.history[self.history.index - 1][0].destroychildren()
            self.history.push(self.history.history[self.history.index - 1][0].up, "up")
